...
SELECT 
    ...
    c.created_at,
    c.updated_at
FROM analytics.entity_campaigns c
...
FROM analytics.entity_customers
...
FROM analytics.entity_customers c
JOIN analytics.entity_subscriptions s ON c.customer_id = s.customer_id
...
FROM analytics.entity_subscriptions s
...
FROM analytics.entity_users u
LEFT JOIN analytics.entity_customers c ON u.customer_id = c.customer_id
...
FROM analytics.entity_subscriptions s
JOIN analytics.entity_users u ON s.sales_rep_id = u.user_id
WHERE s.subscription_created_at >= CURRENT_DATE - INTERVAL '1 year'
...
SELECT 
    ...
    COUNT(DISTINCT c.customer_id)::FLOAT / (SELECT COUNT(DISTINCT customer_id) FROM analytics.entity_customers WHERE is_active = true) as adoption_rate,
...
FROM analytics.entity_features f
JOIN analytics.entity_customers c ON true -- Simplified join for example
...
FROM analytics.entity_users u
CROSS JOIN analytics.entity_features f
...
FROM analytics.entity_devices d
JOIN analytics.entity_locations l ON d.location_id = l.location_id
...
FROM analytics.entity_locations l
...
FROM analytics.entity_locations l
LEFT JOIN analytics.entity_devices d ON l.location_id = d.location_id
...
-- Grant permissions
GRANT SELECT ON ALL TABLES IN SCHEMA mart TO analytics;
GRANT USAGE ON SCHEMA mart TO analytics;
